...
public class CobwebFeature extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        for (int i = 64; i < world.getHeight(); i++) {
            ...
            if (world.getBlockState(mutable).isOf(Blocks.SPRUCE_LEAVES) && world.getBlockState(mutable.down()).isAir() && random.nextBoolean() && FeatureHelper.isCorrectBiome(world.getBiomeKey(mutable).get())) {
