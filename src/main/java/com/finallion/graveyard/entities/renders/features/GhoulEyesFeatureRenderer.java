...
public class GhoulEyesFeatureRenderer extends GeoLayerRenderer<BaseGhoulEntity> {
    private RenderLayer TEXTURE;
    private final IGeoRenderer<BaseGhoulEntity> renderer;

    public GhoulEyesFeatureRenderer(IGeoRenderer<BaseGhoulEntity> entityRendererIn) {
        super(entityRendererIn);
        this.renderer = entityRendererIn;
    }

    @Override
    public void render(MatrixStack matrixStackIn, VertexConsumerProvider bufferIn, int packedLightIn, BaseGhoulEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
