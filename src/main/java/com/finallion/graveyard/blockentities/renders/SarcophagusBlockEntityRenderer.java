...
public class SarcophagusBlockEntityRenderer<T extends BlockEntity & ChestAnimationProgress> implements BlockEntityRenderer<SarcophagusBlockEntity> {
    ...
    private void render(SarcophagusBlockEntity entity, MatrixStack matrixStack, VertexConsumerProvider vertexConsumer, int light, float g, Random random, BakedModel model, boolean isLid) {
        ...
        if (isLid) {
            matrixStack.multiply(Vec3f.NEGATIVE_Z.getDegreesQuaternion(g * 40)); // lid rotation, originally 70
            //matrices.translate(isFoot ? -(openFactor * 0.25) : openFactor * 0.25, openFactor * 0.25, 0.0F);
            matrixStack.translate(g * 0.25, g * 0.25, 0.0F); // moves lid away from body
        }
